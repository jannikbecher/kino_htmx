export function init(ctx, payload) {
  ...
  <label for="available-assigns">mount</label>
  <div contenteditable name="available-assigns" id="component-assigns"></div>
  <label for="html">render</label>
  ...
  componentAssignsEl.innerText = payload.assigns;
  componentAssignsEl.addEventListener("change", (event) => {
    ctx.pushEvent("update", { assigns: event.target.innerText });
  });
  ...
  ctx.handleEvent("update", ({ type, path, assigns, html }) => {
    ...
    componentAssignsEl.innerText = assigns;
    textarea.value = html;
  });
